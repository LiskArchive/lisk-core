	...
	stages {
		...
		stage('Delpoy network') {
			steps {
				retry(5) {
					ansiColor('xterm') {
						ansibleTower credential: '', extraVars: """NEWRELIC_ENABLED: '${params.NEWRELIC_ENABLED}' devnet: ${params.NETWORK} do_nodes_per_region: ${params.NODES_PER_REGION} jenkins_ci: 'yes' lisk_version: ${env.LISK_VERSION}""", importTowerLogs: true, importWorkflowChildLogs: false, inventory: '', jobTags: '', jobTemplate: '46', jobType: 'run', limit: '', removeColor: false, skipJobTags: '', templateType: 'job', throwExceptionWhenFail: true, towerServer: 'tower', verbose: false
					}
				}
			}
		}
				...
				timestamps {
					nvm(getNodejsVersion()) {
						ansiColor('xterm') {
							...
							sh 'npm run stress:diversified | true'
						}
					}
				}
			}
