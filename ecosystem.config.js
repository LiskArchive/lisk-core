...
module.exports = {
	...
	deploy: {
		local: {
			...
			'post-setup': 'source ~/.bashrc; source ~/.zshrc; nvm install && yarn install --frozen-lockfile --ignore-engines && npm run build',
		...
		remote: {
			...
			path: '/tmp/lisk-core',
			'pre-setup': 'rm -rf /tmp/lisk-core',
			'post-setup': 'source ~/.bashrc; nvm install && yarn install --frozen-lockfile --ignore-engines && npm run build',
