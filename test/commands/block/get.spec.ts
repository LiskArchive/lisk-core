...
describe('block:get command', () => {
  ...
  const blockDataAtHeightTwo = {
    header: {
      ...
      reward: "0",
      signature: "WqNtAMvNE1tVSEwXuonajsusTfLMwsDBK52wz05Ix0EixsjVEAz4P6g/edVoTszy755sVUCLrJ3qRcK1qlkKDA==",
  ...
  const encodedBlockData = '';
  ...
  describe('block:get', () => {
    setupTest()
      .command(['block:get'])
      .catch((error: Error) => expect(error.message).to.contain('Please provide id or height of the block to be fetched'))
      .it('should throw an error when no flag is provided');
  });

  describe('block:get --id', () => {
    setupTest()
      .command(['block:get', '--id=T35B9XRMDCpDTxOvsYa3f7SxdqUpj5HthmaA/17xOm0='])
  ...
  describe('block:get --height', () => {
    ipcInvokeStub
      .withArgs('app:getBlockByHeight', { height })
      .resolves(encodedBlockData);
    setupTest()
      .command(['block:get', '--height=2'])
      .it('should get block info and display as an object', () => {
        expect(ipcInvokeStub).to.have.been.calledTwice;
        expect(ipcInvokeStub).to.have.been.calledWithExactly('app:getSchema');
        expect(ipcInvokeStub).to.have.been.calledWithExactly('app:getBlockByID', { id: blockId });
