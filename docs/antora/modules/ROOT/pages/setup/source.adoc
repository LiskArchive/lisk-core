...
git checkout v4.0.0
----
NOTE: Please check for the latest release in {url_core_releases}[Lisk Core releases^].
...
yarn build
----
Set up the `lisk-core` alias: Append the following line to your `~/.profile` file and run `source ~/.profile`.
[source,bash]
----
alias lisk-core=/home/lisk/lisk-core/bin/run
----
NOTE: In the command above, please replace `/home/lisk/lisk-core` with the absolute path to your "lisk-core" directory. To find the exact path on your system, change to your "lisk-core" directory and run the `pwd` command.
