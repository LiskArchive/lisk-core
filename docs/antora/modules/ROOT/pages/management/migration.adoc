...
:lisk-migrator: lisk-migrator-v2.0.0-rc.1-linux-x64.tar.gz
:lisk-migrator_darwinx64: lisk-migrator-v2.0.0-rc.1-darwin-x64.tar.gz
:lisk-migrator_darwinarm64: lisk-migrator-v2.0.0-rc.1-darwin-arm64.tar.gz
...
[TIP]
====
In case a validator node does not fulfill the hardware requirements of the lisk-migrator, it is possible to run the migrator on a local computer.
...
.. DO NOT enable block generation on the Lisk Core v3 node on your local machine, your validator should still be generating blocks on the remote Lisk Core v3 node.
. In step <<wait-until-the-network-reaches-the-snapshot-height>>, invoke the xref:{url_api_forgingstatus}[app:getForgingStatus] endpoint on the remote Lisk Core v3 node, as soon as the snapshot height is reached, and note down the returned values. We recommend automating it with a script to ensure precision.
...
[source,bash]
----
lisk-core keys:create --chainid 00000000 --output ./config/keys.json --add-legacy
----
--
Testnet::
+
...
[source,bash]
----
lisk-core keys:create --chainid 01000000 --output ./config/keys.json --add-legacy
----
--
====
...
To verify, that the validator keys were imported as intended, invoke the `pos_getValidator` endpoint with your validator address as parameter:
...
Look in the output directory of the lisk-migrator for a file called `forgingStatus.json`, this will contain the valid validator info data to enable block generation on the Lisk Core v4 node.
Alternatively, the relevant data can be obtained as well by fetching the necessary values from Lisk Service, after the snapshot height has been reached.
Please note that the `generatorAddress` needs to be updated by the user to the appropriate address:

https://testnet-service.lisk.com/api/v2/blocks?generatorAddress=lskbgyrx3v76jxowgkgthu9yaf3dr29wqxbtxz8yp&height=:20449414&limit=1[^]
...
Replace the address `lskqaxxmj78frvgpjgwvf4yqjjkcrr9yhn2sxxwm3` with your validator address, and set the values for `height`, `max-height-generated` and `max-height-prevoted` to the values listed in the `forgingStatus.json` file corresponding your address.
...
[source,bash]
----
lisk-core transaction:create legacy registerKeys 10000000 --send
----
