...
:lisk-migrator: lisk-migrator-v2.0.0-rc.0-linux-x64.tar.gz
:lisk-migrator_darwinx64: lisk-migrator-v2.0.0-rc.0-darwin-x64.tar.gz
:lisk-migrator_darwinarm64: lisk-migrator-v2.0.0-rc.0-darwin-arm64.tar.gz
...
=== Ensure you are running version 3.1.0 of Lisk Core
Ensure you are running version 3.1.0 of Lisk Core to be able to seamlessly migrate to Lisk Core 4.0.0.
...
If your version is lower than 3.1.0, ensure to update Lisk Core before proceeding with the migration.
...
=== Add the snapshot height to the node configuration

Open the `config.json` of your Lisk Core node and add a new property `backup.height` and assign it the snapshot height of the respective network.
...
Restart the node with the `--overwrite-config` flag to apply the new config.
...
[NOTE]
====
If you migrate the existing v3 config with the `--auto-migrate-config` flag in the next step <<run-lisk-migrator>>, please ensure that the necessary ports in the final v4 config are open for communication.

The final config (when auto-migrating) is printed on the screen for user confirmation, can check the exact port details there.
...
//TODO: Is 7887 optional or mandatory to open?
...
----
lisk-migrator --snapshot-height {snapshotHeight} --output ~/.lisk/lisk-core/config --auto-migrate-config --auto-start-lisk-core-v4
----
--
Testnet::
+
...
[source,bash,subs=attributes+]
----
lisk-migrator --snapshot-height {snapshotHeightTestnet} --output ~/.lisk/lisk-core/config  --auto-migrate-config --auto-start-lisk-core-v4
----
--
====
...
[source,bash,subs=attributes+]
----
lisk-migrator --snapshot-height {snapshotHeight} --output ~/.lisk/lisk-core/config/mainnet --lisk-core-v3-data-path ~/lisk/custom/path/  --auto-migrate-config --auto-start-lisk-core-v4
----
====
...
[TIP]
====
In case a xref:{url_build_custom_config}[custom config] is used, then it is possible to define the path to the custom config file with the `--config` flag like so:

[source,bash,subs=attributes+]
----
lisk-migrator --snapshot-height {snapshotHeight} --output ~/.lisk/lisk-core/config/mainnet --config=/path/to/config.json --auto-migrate-config --auto-start-lisk-core-v4
----
====
...
[source,bash,subs=attributes+]
----
screen -dmSL migration lisk-migrator --snapshot-height {snapshotHeight} --output ~/.lisk/lisk-core/config/mainnet  --auto-migrate-config --auto-start-lisk-core-v4
----
...
.Running the migrator on a local machine
[TIP]
====
In case a validator node does not fulfill the hardware requirements of lisk-migrator, it is possible to run the migrator on a local computer.

IMPORTANT: This approach should only be followed, if it is not possible to acquire the required hardware resources on a remote server, as it is quite cumbersome.

If you still need to perform the migration on your local machine, these are the recommended steps to do so:

. Follow all steps under <<preparation>> on your local machine.
.. DO NOT enable forging on the Lisk Core v3 node on your local machine, your validator should still be forging on the remote Lisk Core v3 node.
. In step <<wait-until-the-network-reaches-the-snapshot-height>>, invoke the xref:{url_api_forgingstatus}[app:getForgingStatus] endpoint on the remote Lisk Core v3 node, as soon as the snapshot height is reached, and note down the returned values.
. Set up Lisk Core v4 manually on your remote server, and let it sync from the genesis block which was generated by the lisk-migrator.
Now follow the remaining <<migration-steps>>.
.. When you reach the point where you wish to enable block generation for your validator, use the validator data that you noted down in step 2.
...
This can take *~20-25 mins* against the *Testnet*, and *~7-7.5 hours* against the *Mainnet*.
...
First, look in the output directory of the lisk-migrator for a file called `forgingStatus.json`, this will contain the valid validator info data to enable block generation on the Lisk Core v4 node.
[source,bash]
----
...
Replace the address `lskqaxxmj78frvgpjgwvf4yqjjkcrr9yhn2sxxwm3` with your validator address, and set the values for `height`, `max-height-generated` and `max-height-prevoted` to the values listed in the `forgingStatus.json` file.
