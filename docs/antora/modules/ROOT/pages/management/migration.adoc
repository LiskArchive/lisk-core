...
//TODO: Update URL with new releases
...
//TODO: This section can be removed after release of v3.1.0-rc.1
=== Add the snapshot height to the node configuration

Open the `config.json` of your Lisk Core node, add a new property `backup.height`, and assign it the snapshot height of the respective network.
...
From `forgingStatus.json` -> `generator:enable` command::
* height -> `--height`
* maxHeightPrevoted -> `--max-height-prevoted`
* maxHeightPreviouslyForged -> `--max-height-generated`
