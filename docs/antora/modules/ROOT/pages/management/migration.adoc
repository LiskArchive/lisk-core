...
Observe if `lisk-migrator` finishes successfully, (this takes about several minutes).
...
For validators, this might result in them missing a block, for everyone else it will not have any impact.
...
////
=== Enabling block generators after migration (for validators)
...
IMPORTANT: The registration of the legacy validator keys has to be done latest at the end of `initRounds`, otherwise, the validator will miss block rewards.

To register the legacy validator keys with the new node, utilize the `registerKeys` command of the Legacy module.

==== Enable block generation

The final step is to enable block generation on the node for your validator.
