...
:lisk-migrator: lisk-migrator-v2.0.0-rc.5-linux-x64.tar.gz
:lisk-migrator_darwinx64: lisk-migrator-v2.0.0-rc.5-darwin-x64.tar.gz
:lisk-migrator_darwinarm64: lisk-migrator-v2.0.0-rc.5-darwin-arm64.tar.gz
...
The new genesis block is created based on a snapshot of the old blockchain (running on Lisk Core v3.1.1) at a given height.
...
Optionally, anyone running a Lisk Core v3.1.1 who is not an active validator can also participate in the migration process.
...
=== Ensure you are running version 3.1.1 of Lisk Core
Ensure you are running version 3.1.1 of Lisk Core to be able to seamlessly migrate to Lisk Core 4.0.0.
...
NOTE: If your version is 3.1.1-rc.0, please ensure to set `backup.height` to the announced snapshot height in your custom config and restart the node with the `--overwrite-config` flag.
...
=== Ensure Lisk Core v3.1.1 is fully synced with the network
...
[source,bash]
----
grep \"id\": ./lisk-migrator/output/15f0dacc1060e91818224a94286b13aa04279c640bd5d6f193182031d133df7c/genesis_block.json
----
...
=== Stop Lisk Core v3.1.1

After the `lisk-migrator` script has finished and the announced snapshot height has passed, there is no reason to continue running Lisk Core v3.1.1, and therefore it is recommended to stop it.
...
////
Navigate into the root folder of your Lisk Core v3.1.1 installation and run the following command to stop the old Lisk Core version:
...
Last but not least, remove the folder with Lisk Core v3.1.1.
...
.Alternative in case of lost data
