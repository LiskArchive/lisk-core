...
After the snapshot height is reached, validators have approximately 2 hours time to enable block generation, to ensure they will not miss any blocks after the hard fork.
...
=== Enabling block generation after the migration (for validators)
