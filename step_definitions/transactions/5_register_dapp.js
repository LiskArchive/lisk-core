...
When('{string} uses second signature account to register for dApp {string}', async (userName, dAppName) => {
  const { passphrase, secondPassphrase } = getFixtureUser('username', userName);
  options = {
    name: dAppName,
    category: 1,
    description: `dApp for ${dAppName}`,
...
When('{string} uses multi signature account to register for dApp {string}', async (userName, dAppName) => {
  const { passphrase } = getFixtureUser('username', userName);
  options = {
    name: dAppName,
    category: 1,
...
When('{string}, {string} send signatures for dApp {string}', async (user1, user2, dAppName) => {
  const isRegistered = await I.checkIfdAppRegistered(dAppName);
  if (!isRegistered) {
