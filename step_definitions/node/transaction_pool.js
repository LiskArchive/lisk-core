...
When('{string} send {int} LSK token to {int} random accounts', async (userName, amount, walletCount) => {
  ...
  await from(Promise.all(trxs.map(t => I.broadcastAndValidateTransaction(t))));
});

Then('I should get list of transactions in {string}, {string} queue', async (state1, state2) => {
...
When('{string} sends {int} LSK token to a random account', async (userName, amount) => {
  const { passphrase } = getFixtureUser('username', userName);
...
Then('multisignature transaction should get confirmed', async () => {
  ...
  expect(result.data[0].signatures).to.deep.equal(signatures.map(s => s.signature));
});
