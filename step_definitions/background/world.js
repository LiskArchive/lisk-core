...
Given("{int} lisk accounts exists with minimum balance", async count => {
  ...
  const randomAccounts = new Array(count)
    .fill(0)
    .map(async () => await I.createAccount());
...
Given(
  ...
  async function(userName, balance) {
...
Given("{string} has a account registered as delegate", async function(
  userName
) {
...
Given(
  ...
  async function(user1, user2, user3) {
